...
def suma_cubo_pares_for(numeros: Iterable[int]) -> int:
    ...
    """
    # Solución
    # Lista con los numeros elevados al cubo
    lista_cubo = []
...
def suma_cubo_pares_sum_list(numeros: Iterable[int]) -> int:
    ...
    #lista_cubo = (list( map(lambda x: x**3, numeros)))
    #lista_pares = list(filter(lambda x: x % 2 == 0 , lista_cubo))
